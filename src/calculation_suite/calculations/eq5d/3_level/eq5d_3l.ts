...
const ALLOWED_DOMAIN_SCORES = [1, 2, 3]

const calculate_eq5d_3l_score = (
  ...
  const calculation_input_eq_5d_vas = R.filter(
    input => R.view(inputIdLens, input) === 'eq5d_3l_vas',
    calculation_input
  )
  ...
  if (
    ...
    const mobilityHealthState = get_domain_value('eq5d_3l_mobility')(
      calculation_input_eq5d_health_state
    )
    const selfCareHealthState = get_domain_value('eq5d_3l_selfcare')(
      calculation_input_eq5d_health_state
    )
    const usualActivitiesHealthState = get_domain_value('eq5d_3l_usual')(
      calculation_input_eq5d_health_state
    )
    const painHealthState = get_domain_value('eq5d_3l_pain')(
      calculation_input_eq5d_health_state
    )
    const anxietyHealthState = get_domain_value('eq5d_3l_anxiety')(
      calculation_input_eq5d_health_state
    )
  ...
  if (do_all_required_inputs_have_a_valid_value(calculation_input_eq_5d_vas)) {
     ...
     * In this array, however, there's only one item (Question "eq5d_3l_vas").
     */
    //@ts-expect-error to do
...
export const specific_steps_eq5d_3l_calc = [
  calculate_eq5d_3l_score,
  add_raw_values_to_inputs(EQ5D_3L_INPUTS),
]

export const eq5d_3l: CalculationType = create_calculation({
  calculation_name: `EQ-5D-3L`,
  readme_location: __dirname,
  calculation_steps: specific_steps_eq5d_3l_calc,
  calculation_definition: {
    input_definition: EQ5D_3L_INPUTS,
    output_definition: EQ5D_3L_OUTPUT,
  },
})
