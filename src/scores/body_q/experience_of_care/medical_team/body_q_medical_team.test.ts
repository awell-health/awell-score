...
import { body_q_medical_team } from './body_q_medical_team'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/medical_team_test_responses'

const score = new Score(body_q_medical_team)

describe('body_q_medical_team', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_medical_team')
  })
  ...
  describe('basic assumptions', function () {
    ...
    it('should have the expected result ids', function () {
      expect(Object.keys(outcome)).toEqual(['BODY_Q_MEDICAL_TEAM_SCORE'])
    })
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({ payload: { BODY_Q_MEDICAL_TEAM_Q01: 99 } }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null for all outputs when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_MEDICAL_TEAM_SCORE).toEqual(null)
    })
  })
  ...
  describe('mean imputation for missing items', function () {
    it('should calculate score using mean imputation when < 50% of items are missing', function () {
      ...
      expect(outcome.BODY_Q_MEDICAL_TEAM_SCORE).toEqual(46)
    })
