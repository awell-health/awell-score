...
import { body_q_eating_behavior } from './body_q_eating_behavior'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/eating_behavior_test_responses'

const score = new Score(body_q_eating_behavior)

describe('body_q_eating_behavior', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_eating_behavior')
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({
          payload: { BODY_Q_EATING_BEHAVIOR_Q01: 99 },
        }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_EATING_BEHAVIOR_SCORE).toEqual(null)
    })
  })
  ...
  describe('mean imputation for missing items', function () {
    it('should impute missing values with the mean and return a valid score when <50% items are missing', function () {
      ...
      expect(outcome.BODY_Q_EATING_BEHAVIOR_SCORE).toEqual(50)
    })
