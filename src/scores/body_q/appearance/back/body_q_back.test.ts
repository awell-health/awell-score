...
import { body_q_back } from './body_q_back'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/back_test_responses'

const score = new Score(body_q_back)

describe('body_q_back', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_back')
  })
  ...
  describe('basic assumptions', function () {
    ...
    it('should have the expected result ids', function () {
      expect(Object.keys(outcome)).toEqual(['BODY_Q_BACK_SCORE'])
    })
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({ payload: { BODY_Q_BACK_Q01: 99 } }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null for all outputs when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_BACK_SCORE).toEqual(null)
    })
  })
  ...
  describe('missing data handling', function () {
    it('should impute missing values with mean when less than 50% missing', function () {
      ...
      expect(outcome.BODY_Q_BACK_SCORE).toEqual(66)
    })
