...
import { body_q_inner_thighs } from './body_q_inner_thighs'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/inner_thighs_test_responses'

const score = new Score(body_q_inner_thighs)

describe('body_q_inner_thighs', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_inner_thighs')
  })
  ...
  describe('basic assumptions', function () {
    ...
    it('should have the expected result ids', function () {
      expect(Object.keys(outcome)).toEqual(['BODY_Q_INNER_THIGHS_SCORE'])
    })
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({ payload: { BODY_Q_INNER_THIGHS_Q01: 99 } }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null for all outputs when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_INNER_THIGHS_SCORE).toEqual(null)
    })
  })
