...
import { body_q_hips_outer_thighs } from './body_q_hips_outer_thighs'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/hips_outer_thighs_test_responses'

const score = new Score(body_q_hips_outer_thighs)

describe('body_q_hips_outer_thighs', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_hips_outer_thighs')
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({
          payload: { BODY_Q_HIPS_OUTER_THIGHS_Q01: 99 },
        }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null for all outputs when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_HIPS_OUTER_THIGHS_SCORE).toEqual(null)
    })
  })
