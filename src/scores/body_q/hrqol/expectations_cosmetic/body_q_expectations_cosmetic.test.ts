...
import { body_q_expectations_cosmetic } from './body_q_expectations_cosmetic'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/expectations_cosmetic_test_responses'

const score = new Score(body_q_expectations_cosmetic)

describe('body_q_expectations_cosmetic', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_expectations_cosmetic')
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({
          payload: { BODY_Q_EXPECTATIONS_COSMETIC_Q01: 99 },
        }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_EXPECTATIONS_COSMETIC_SCORE).toEqual(null)
    })
  })
  ...
  describe('mean imputation for missing items', function () {
    it('should impute missing values with the mean and return a valid score when <50% items are missing', function () {
      ...
      expect(outcome.BODY_Q_EXPECTATIONS_COSMETIC_SCORE).toEqual(47)
    })
