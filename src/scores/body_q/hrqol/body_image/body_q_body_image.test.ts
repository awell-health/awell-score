...
import { body_q_body_image } from './body_q_body_image'
import {
  best_response,
  worst_response,
  random_response,
...
} from './__testdata__/body_image_test_responses'

const score = new Score(body_q_body_image)

describe('body_q_body_image', function () {
  it('should be available in the ScoreLibrary', function () {
    expect(ScoreLibrary).toHaveProperty('body_q_body_image')
  })
  ...
  describe('basic assumptions', function () {
    ...
    it('should have the expected result ids', function () {
      expect(Object.keys(outcome)).toEqual(['BODY_Q_BODY_IMAGE_SCORE'])
    })
  })
  ...
  describe('validation', function () {
    ...
    it('should throw ZodError for invalid input', function () {
      expect(() =>
        score.calculate({
          payload: { BODY_Q_BODY_IMAGE_Q01: 99 },
        }),
      ).toThrow(ZodError)
    })
    ...
    it('should return null when called with empty payload', function () {
      ...
      expect(outcome.BODY_Q_BODY_IMAGE_SCORE).toEqual(null)
    })
  })
  ...
  describe('mean imputation for missing items', function () {
    it('should impute missing values with the mean and return a valid score when <50% items are missing', function () {
      ...
      expect(outcome.BODY_Q_BODY_IMAGE_SCORE).toEqual(50)
    })
